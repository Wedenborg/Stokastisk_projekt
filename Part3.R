...
while (i<=10){
  ...
  if (Y[apply(Y, 1, function(x, want) isTRUE(all.equal(x, want)), Y_test),]==T)
    count[which(apply(Y, 1, function(x, want) isTRUE(all.equal(x, want)), Y_test), arr.ind=TRUE)[1]]=Y_test
    Y[apply(Y, 1, function(x, want) isTRUE(all.equal(x, want)), Y_test),]=0
  if (Y_test == all(Y[,])){
    i = i+1
    print(i)
  } else {
